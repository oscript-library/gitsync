...
Процедура Инициализация()
	...
	Распаковщик.УстановитьКоличествоЦикловОжиданияЛицензииПоУмолчанию(1);
...
Функция ПолучитьСписокТестов(Знач Контекст) Экспорт
	...
	ВсеТесты.Добавить("Тест_ДолженПереместитьВКаталогРабочейКопии");
...
Процедура Тест_ДолженВыполнитьПолнуюВыгрузку() Экспорт
	...
	ПутьКХранилищу = ПодготовитьВременноеХранилище();
	
	Распаковщик.ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(КаталогВыгрузки, ПутьКХранилищу, 1, РежимВыгрузкиФайлов.Плоский);
...
Процедура Тест_ДолженПереместитьВКаталогРабочейКопии() Экспорт
	
	КаталогРабочейКопии = ВременныеФайлы.СоздатьКаталог();
	ПутьКХранилищу = ПодготовитьВременноеХранилище();
	Формат = РежимВыгрузкиФайлов.Плоский;
	Конфигуратор = Распаковщик.ПолучитьМенеджерКонфигуратора();
	Конфигуратор.ИспользоватьВерсиюПлатформы("8.3");
	
	Распаковщик.ЗагрузитьВерсиюХранилищаВКонфигурацию(Конфигуратор, КаталогРабочейКопии, ПутьКХранилищу, 1);
	
	КаталогВыгрузки = ВременныеФайлы.СоздатьКаталог();
	
	Распаковщик.ВыгрузитьКонфигурациюВИсходники(Конфигуратор, КаталогВыгрузки, Формат);
	
	Распаковщик.ОчиститьКаталогРабочейКопии(КаталогРабочейКопии);

	Распаковщик.ПереместитьВКаталогРабочейКопии(КаталогРабочейКопии, КаталогВыгрузки, Формат);
		
	ФайлПереименований = Новый Файл(ОбъединитьПути(КаталогРабочейКопии,"renames.txt"));
	...
	МассивФайлов = НайтиФайлы(КаталогРабочейКопии,"*.*");
	Утверждения.ПроверитьИстину(МассивФайлов.Количество() > 0, "в каталоге разложения модулей 1С <"+КаталогРабочейКопии+"> должны быть файлы");
...
Процедура Тест_ДолженПрочитатьФайлВерсийСИменамиПользователейИзХранилища1С() Экспорт
	...
	ТаблицаИсторииХранилища = Распаковщик.ПрочитатьИзХранилищаИсториюКоммитовСАвторами(ПутьКФайлуХранилища1С,КаталогРабочейКопии);
...
Процедура ТестДолжен_ПроверитьОтсутствиеФайлаAUTHORS() Экспорт
	...
	ТаблицаИсторииХранилища = Распаковщик.ПрочитатьИзХранилищаИсториюКоммитовСАвторами(ПутьКФайлуХранилища1С, КаталогРепозитория);
...
Функция ПодготовитьВременноеХранилище()
...
КонецФункции // ПодготовитьВременноеХранилище()

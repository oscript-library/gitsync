...
Процедура ЯИнициализируюСвязьСВнешнимРепозиторием(Знач ПарамСтрока1, Знач ПарамСтрока2) Экспорт
	...
	ГитРепозиторий = БДД.ПолучитьИзКонтекста("ГитРепозиторий");
	URLРепозитория = БДД.ПолучитьИзКонтекста(ПарамСтрока2);
	ПутьКаталогаИсходников = БДД.ПолучитьИзКонтекста(ПарамСтрока1);
...
Процедура ЯУстанавливаюПутьВыполненияКомандыКТекущейБиблиотеке(Знач ИмяКоманды) Экспорт
	...
	СтрокаКоманды = СтрШаблон("oscript %1 %2", "-encoding=utf-8",  ПутьГитсинк);
	
	Команда = БДД.ПолучитьИзКонтекста(КлючКоманды(ИмяКоманды));
	СтрокаКоманды = ОбернутьВКавычки(СтрокаКоманды);
	Команда.УстановитьКоманду(ОбернутьВКавычки(СтрокаКоманды));

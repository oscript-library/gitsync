...
Сценарий: Cинхронизация с использованием push
    Допустим Я добавляю параметр "-p push" для команды "gitsync"
...
Сценарий: Cинхронизация c использованием push-tags
    Допустим Я добавляю параметр "-p pull" для команды "gitsync"
    И Я добавляю параметр "-p push" для команды "gitsync"
    И Я добавляю параметр "sync" для команды "gitsync"
...
Сценарий: Cинхронизация c использованием push-every-n-commits
    Допустим Я добавляю параметр "-p push" для команды "gitsync"
    И Я добавляю параметр "sync" для команды "gitsync"
    И Я добавляю параметр "--push-every-n-commits 1" для команды "gitsync"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "КаталогХранилища1С"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "ПутьКаталогаИсходников"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "URLРепозитория"
    Когда Я выполняю команду "gitsync"

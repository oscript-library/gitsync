...
Функционал: Работа плагина использования механизма 1С Предприятие для получения версии из хранилища конфигурации
    Как Пользователь
...
Контекст: Тестовый контекст
    ...
    И я включаю отладку лога с именем "oscript.app.gitsync_plugins"
    И я включаю отладку лога с именем "oscript.app.gitsync_plugins_vendorUpload"
    И Я создаю временный каталог и сохраняю его в контекст
    И Я сохраняю значение временного каталога в переменной "ВременнаяДиректория"
    И Я добавляю параметр "--tempdir" для команды "gitsync" из переменной "ВременнаяДиректория"

Сценарий: Cинхронизация с использованием vendorUpload
    Допустим Я добавляю параметр "-p vendorUpload" для команды "gitsync"
...
Сценарий: Cинхронизация с использованием vendorUpload с получением истории через 1С
    Допустим Я добавляю параметр "-p vendorUpload" для команды "gitsync"
    И Я добавляю параметр "sync" для команды "gitsync"
    И Я добавляю параметры для команды "gitsync"
    |-u Администратор|

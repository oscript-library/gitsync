...
Контекст: Тестовый контекст
    ...
    И я включаю отладку лога с именем "oscript.app.gitsync.plugins.check-comments"
...
Сценарий: Cинхронизация с использованием просто check-comments
    Допустим Я добавляю параметр "-plugins check-comments" для команды "gitsync" 
...
Сценарий: Cинхронизация c использованием call-error
    Допустим Я добавляю параметр "-call-error" для команды "gitsync" 
    И Я добавляю параметр "-plugins check-comments" для команды "gitsync" 
    Когда Я выполняю команду "gitsync"
    Тогда Вывод команды "gitsync" содержит "КРИТИЧНАЯОШИБКА - Нашли следующую версию <4> от автора <Администратор>, а комментарий не задан!"
    И Вывод команды "gitsync" не содержит "Внешнее исключение"
    И Код возврата команды "gitsync" равен 1

Сценарий: Cинхронизация c использованием send-email
    Допустим Я добавляю параметр "-send-email" для команды "gitsync" 
    И Я добавляю параметр "-plugins check-comments" для команды "gitsync" 
    Когда Я выполняю команду "gitsync"
...
Сценарий: Cинхронизация хранилища все вместе
    Допустим Я добавляю параметр "-send-email" для команды "gitsync" 
    И Я добавляю параметр "-add-emails test@mail.ru" для команды "gitsync" 
    И Я добавляю параметр "-call-error" для команды "gitsync" 
    И Я добавляю параметр "-plugins check-comments" для команды "gitsync" 
    Когда Я выполняю команду "gitsync"
    Тогда Вывод команды "gitsync" содержит "ИНФОРМАЦИЯ - Синхронизация завершена"
    И Вывод команды "gitsync" не содержит "Внешнее исключение"

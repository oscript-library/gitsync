...
Процедура ДобавитьКомандуAuthors(Знач Парсер)

	Команда = Парсер.ОписаниеКоманды("convert-authors", "Выполнить конвертацию с файлом authors, без pull/push");
	...
	Команда = Парсер.ОписаниеКоманды("init-authors", "Инициализация файла authors, без pull/push");
...
Процедура ДобавитьАргументыПоУмолчанию(Знач Парсер)
	...
	Парсер.ДобавитьИменованныйПараметр("-useautotags", "<on|off>");
...
Процедура ВыполнитьКоманду(Знач ОписаниеКоманды)
	...
	ИначеЕсли ОписаниеКоманды.Команда = "convert-authors" Тогда
		ВыполнитьКомандуКонвертацииAUTHORS(Параметры);
	ИначеЕсли ОписаниеКоманды.Команда = "init-authors" Тогда
		ВыполнитьКомандуИнициализацииAUTHORS(Параметры);
	Иначе
...
Процедура ВыполнитьКомандуИнициализацииAUTHORS(Знач Параметры)

	Распаковщик = ПолучитьИНастроитьРаспаковщик(Параметры);
	КаталогРабочейКопии = Параметры["ЛокальныйКаталогГит"];
	ПутьКХранилищу = Параметры["ПутьКХранилищу"];

	Распаковщик.ИнициализоватьмAUTHORS();
	ПутьКФайлуAUTHORS = Распаковщик.ПолучитьПутьКФайлуAUTHORS(КаталогРабочейКопии);
	ФайлуAUTHORS = Новый Файл(ПутьКФайлуAUTHORS);
	
	ТаблицаUSERS = Распаковщик.ПрочитатьТаблицуПользователейХранилища(ПолучитьПутьКБазеДанныхХранилища(ПутьКХранилищу));
	ДанныеUSERS = Распаковщик.КонвертироватьВJSONТаблицуUSERSИзФорматаБД(ТаблицаUSERS);

	Если ФайлуAUTHORS.Существует() Тогда
		Распаковщик.ПрочитатьФайлAuthors(ПутьКФайлуAUTHORS);
		Распаковщик.СинхронизирватьAUTHORSUSERS(ДанныеUSERS,ПутьКФайлуAUTHORS);
	Иначе
		Распаковщик.СинхронизирватьAUTHORSUSERS(ДанныеUSERS,ПутьКФайлуAUTHORS);
	КонецЕсли;
...
Процедура ВыполнитьКомандуКонвертацииAUTHORS(Знач Параметры)
	...
	Распаковщик.СконвертироватьAuthorsВФормалJSON(ПутьКФайлуAUTHORS, ДанныеUSERS);
...
Процедура Синхронизировать(Знач ПутьКХранилищу,
	...
	Если АвтоматическаяУстановкаТэговПоВерсиям = "on" Тогда
		АвтоматическаяУстановкаТэговПоВерсиям = истина;
	ИначеЕсли АвтоматическаяУстановкаТэговПоВерсиям = "off" Тогда
		АвтоматическаяУстановкаТэговПоВерсиям = Ложь;
	ИначеЕсли ТипЗнч(АвтоматическаяУстановкаТэговПоВерсиям) = Тип("Булево")Тогда

		АвтоматическаяУстановкаТэговПоВерсиям = АвтоматическаяУстановкаТэговПоВерсиям;
	...
	Лог.Отладка("АвтоматическаяУстановкаТэговПоВерсиям = " + АвтоматическаяУстановкаТэговПоВерсиям);
	...
	ВыполнитьЭкспортИсходников(Распаковщик, ПутьКХранилищу, ЛокальныйКаталогГит, НачальнаяВерсия, КонечнаяВерсия, Формат, КоличествоКоммитовДоPush, URLРепозитория, ИмяВетки, АвтоматическаяУстановкаТэговПоВерсиям);
...
Процедура ВыполнитьЭкспортИсходников(Знач Распаковщик, 
	...
	Распаковщик.СинхронизироватьХранилищеКонфигурацийСГит(ЛокальныйКаталогГит, ФайлБазыДанныхХранилища, НачальнаяВерсия, КонечнаяВерсия, Формат, КоличествоКоммитовДоPush, URLРепозитория, ИмяВетки, АвтоматическаяУстановкаТэговПоВерсиям);

...
Процедура ПодготовитьНовыйРепозитарий(Знач Параметры, Знач РежимИнициализации)
	...
	НаполнитьКаталогРабочейКопииСлужебнымиДанными(КаталогРабочейКопии, Распаковщик, Параметры["ПутьКХранилищу"], Параметры["-firstversion"]);
...
Процедура НаполнитьКаталогРабочейКопииСлужебнымиДанными(Знач КаталогРабочейКопии, Знач Распаковщик, Знач ПутьКХранилищу, Знач НомерВерсии = Неопределено)
	
	Если НомерВерсии = Неопределено Тогда 
		НомерВерсии = ""; //Оставляем пустым для совместимости со старым поведением.
	КонецЕсли;
	...
	СгенерироватьФайлVERSION(КаталогИсходников.ПолноеИмя, Распаковщик, НомерВерсии);
...
Процедура СгенерироватьФайлVERSION(Знач КаталогИсходников, Знач Распаковщик, Знач Версия)
	...
	Распаковщик.ЗаписатьФайлВерсийГит(ФайлВерсий.Путь, Версия);

...
Процедура ВыполнитьПриложение()
	...
	Приложение.Опция("ignore-broken-plugins", Ложь, "разрешить работу приложения при сломанных расширениях")
					.ВОкружении("GITSYNC_IGNORE_BROKEN_PLUGINS");
...
Процедура ПередВыполнениемКоманды(Знач Команда) Экспорт
	...
	Если Команда.ЗначениеОпции("ignore-broken-plugins") = Истина Тогда
		ПараметрыПриложения.УстановитьПризнакПродолженияВыполненияПриОшибкахПлагинов(Истина);
	КонецЕсли;
...
Процедура ПослеВыполненияКоманды(Знач Команда) Экспорт
	...
	// В конце работы выводим предупреждение ещё раз
	ПараметрыПриложения.ПроверитьЗагрузкуПлагинов(Истина);

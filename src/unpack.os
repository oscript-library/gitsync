...
Процедура РазложитьМодули1СПоПапкамСогласноИерархииМетаданных(Знач КаталогПлоскойВыгрузки, Знач КаталогИерархическойВыгрузки, Знач Формат) Экспорт
	...
	Сообщить("Каталога иерархической:"+КаталогИерархическойВыгрузки);
	Для Каждого Файл Из СписокФайлов Цикл
		ИмяФайлаДляПерименования = Файл.Имя; 
		Если Формат = "hierarchical" Тогда
			...
			Если Файл.ЭтоКаталог() Тогда
				...
				Сообщить("Имя нового каталога:"+ИмяКаталога);
				...
				Продолжить;
			КонецЕсли;
			ИмяФайлаДляПерименования = ИмяНовогоФайла;
		Иначе
			Если Файл.ЭтоКаталог() Тогда

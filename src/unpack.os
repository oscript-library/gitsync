...
Процедура ОбработатьКаталогРезультатаВыгрузки(Знач Файл, Знач КаталогИерархическойСтруктурыМодулей, Знач Переименования)
	Если Прав(Файл.Имя, 5) = "files" Тогда // файлы справки. 
		...
		ИмяНовогоФайла = СтрЗаменить(ИмяНовогоФайла, ".", Разделитель)+".del";
	...
	Для Счетчик = 0 По КоличествоВсего-1 Цикл
		...
		Если ДлинаИменни > 58 Тогда
			...
			Иначе
				...
				НовыйПутьПроверки = Новый Файл(ДополнитьСлешВПуть(КаталогИерархическойСтруктурыМодулей)+ИмяФайла+НовоеИмя);
				Если НовыйПутьПроверки.Существует() Тогда
					СчетчикНовогоИмени = 0;
					МассивФайловСущуствующих = НайтиФайлы(ДополнитьСлешВПуть(КаталогИерархическойСтруктурыМодулей)+ИмяФайла, Лев(ЭлементМассива, 58-Разрядность-3)+"*");
...
Функция КлонироватьРепозитарий(Знач КаталогЛокальнойКопии, Знач URLРепозитария) Экспорт
	...
	ВременныеФайлы.УдалитьФайл(ФайлЛога);
...
ПодключитьСценарий(ОбъединитьПути(ТекущийСценарий().Каталог, "cmd-builder.os"), "КомандныйФайл");

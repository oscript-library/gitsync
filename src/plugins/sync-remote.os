...
Процедура ПриРегистрацииКомандыПриложения(ИмяКоманды, КлассРеализации, Парсер) Экспорт
	...
	КлассРеализации.Опция("P push", Ложь, "[*git-remote] Флаг отправки изменений на удаленный репозиторий").Флаг().ВОкружении("GITSYNC_PUSH");
	КлассРеализации.Опция("G pull", Ложь, "[*git-remote] Флаг получения изменений из удаленный репозитория перед синхронизацией").Флаг().ВОкружении("GITSYNC_PULL");
	КлассРеализации.Опция("T push-tags", Ложь, "[*git-remote] Флаг отправки тегов по версиям").Флаг().ВОкружении("GITSYNC_PUSH_TAGS");
	КлассРеализации.Опция("n push-n-commits", 0, "[*git-remote] <число> количество коммитов до промежуточной отправки на удаленный сервер").ТЧисло().ВОкружении("GITSYNC_PUSH_N_COMMITS");

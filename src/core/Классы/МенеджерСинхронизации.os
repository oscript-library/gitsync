...
Функция ВыполнитьGitPush(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки) Экспорт
	...
	// Закомментил т.к. используется настройка для из remote.origin.push
...
Функция ВыполнитьGitPushTags(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки) Экспорт
...
Процедура СинхронизирватьAUTHORSUSERS(Знач ДанныеUSERS, знач ФайлАвторов = Неопределено)
	...
	Для Каждого ПользовательХранилища Из ДанныеUSERS Цикл
			...
			НайденныйПользователь = мСоответствиеUSERIDиЗаписиAUTHORS.Получить(USERID);
			...
			Иначе	
				Лог.Отладка(СтрШаблон("Пользователь с именем: %1 найден UID (%2). 
				|Найденный пользователь:	
				|	Имя: %3
				|	Полное имя: %4
				|	Email: %5", USERKEY, USERID, НайденныйПользователь.Name, НайденныйПользователь.FullName, НайденныйПользователь.Email));
				Продолжить;
			КонецЕсли;
...
Функция ВернутьКлючАвтораПоГУИД(Знач USERID)
	...
	Возврат ?(АвторПоUSERID = Неопределено, ВернутьКлючАвтораПоУмолчанию(), АвторПоUSERID)
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				Если ИспользоватьПромежуточныйPUSH И
					...
					Если АвтоматическаяУстановкаТэговПоВерсиям Тогда
						ВыполнитьGitPushTags(КаталогРабочейКопии, URLРепозитория, ИмяВетки);
					КонецЕсли;

...
Перем ВременнаяИБ; // строка соединения с временной информационной базой вида /F<каталог базы> или /S<сервер>\<база>
...
//
// Параметры:
//   СтрокаСоединения - Строка - строка соединения с временной информационной базой
//                               вида /F<каталог базы> или /S<сервер>\<база>
//
// Возвращаемое значение:
//   Объект.МенеджерСинхронизации - ссылка на текущий объект класса <МенеджерСинхронизации>
//
Функция ВременнаяИБ(Знач СтрокаСоединения = Неопределено) Экспорт
	
	Лог.Отладка("Установлена строка соединения с временной информационной базой <%1>", СтрокаСоединения);
	ВременнаяИБ = СтрокаСоединения;
...
Функция ПолучитьУправлениеКонфигуратором() Экспорт
	...
	Если ЗначениеЗаполнено(ВременнаяИБ) Тогда
		УправлениеКонфигуратором.УстановитьКонтекст(ВременнаяИБ, "", "");
	Иначе

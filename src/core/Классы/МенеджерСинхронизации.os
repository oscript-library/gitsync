...
//
Функция ПолучитьПутьКФайлуAUTHORS(Знач КаталогРепо)
Функция ПрочитатьИзХранилищаИсториюКоммитовСАвторами(Знач ФайлХранилища, Знач КаталогРепо) Экспорт
	...
	СинхронизирватьAUTHORSUSERS(МассивПользователей, ПолучитьПутьКФайлуAUTHORS(КаталогРепо))
...
Функция КонвертироватьВJSONТаблицуUSERSИзФорматаБД(Знач ТаблицаБД)
...
Процедура СконвертироватьAuthorsВФормалJSON(Знач ФайлАвторов, Знач ДанныеUSERS)
	...
	СтруктураUSERSПоКлючуNAME = Новый Структура;
	...
	Попытка
		...
		Пока СтрокаФайла <> Неопределено Цикл
			...
			Если Индекс > 0 Тогда
				...
				Имя = Сред(СтрокаФайла,Индекс+1, ИндексНачалаEmail- Индекс);
				Email = Сред(СтрокаФайла, ИндексНачалаEmail+1, ИндексКонецEmail - ИндексНачалаEmail);
				
				НовыйАвтор = Новый Структура
				НовыйАвтор.Вставить("Имя", ДанныеПользователяХранилища.NAME)
				НовыйАвтор.Вставить("Email", Email);

				ДанныеAUTHORS.Вставить(USERID, НовыйАвтор)
	...
	ЗаписатьФайлAuthors(ФайлАвторов, ДанныеAUTHORS);
...
Процедура ЗаписатьФайлAuthors(Знач ФайлАвторов, Знач ДанныеAUTHORS = Неопределено)
	...
	ПарсерJSON = Новый ПарсерJSON;
	ТекстФайлАвторов = ПарсерJSON.ЗаписатьJSON(ДанныеAUTHORS);

	ЗаписатьДанныеВФайл(ТекстФайлАвторов, ФайлАвторов);

КонецПроцедуры

Процедура ПрочитатьДанныеИзФайла(Знач ПутькФайлу)
	
	ТекстовыеДанные = "";

	Попытка
		ТекстовойДокумент = Новый ТекстовыйДокумент;
		ТекстовыеДанные = ТекстовойДокумент.Прочитать(ПутькФайлу);
...
Процедура УстановитьАвтораПоУмолчанию(Знач ИмяАвтора = Неопределено, Знач Email = Неопределено)
	...
	Если Email = Неопределено Тогда
		...
		Email = СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонEmail, НовыйАвтор.Имя, ДоменПочтыДляGit);
	КонецЕсли;

	НовыйАвтор = Новый Структура
	НовыйАвтор.Вставить("Имя", ИмяАвтора)
	НовыйАвтор.Вставить("Email", ;
...
Процедура ЗаписатьДанныеВФайл(Знач ТекстовыеДанные, Знач ПутькФайлу)
	...
	Попытка
		ТекстовойДокумент = Новый ТекстовыйДокумент;
		ТекстовойДокумент.УстановитьТекст(Данные);
		ТекстовойДокумент.Записать(ТекстовыеДанные,"utf-8");
...
Процедура СинхронизирватьAUTHORSUSERS(Знач ДанныеUSERS, знач ФайлАвторов = Неопределено)
	...
	Для Каждого ПользовательХранилища Из ДанныеUSERS Цикл
			...
			Если Не мAUTHORS.Получить(ПользовательХранилища.USERID) = Неопределено Тогда
				Продолжить;
			КонецЕсли;

			мAUTHORS.Вставить(
								ПользовательХранилища.USERID,
	...
	Если ЕстьИзмененияВФайлеАвторов И Не ФайлАвторов = Неопределено Тогда ЗаписатьФайлAuthors(ФайлАвторов) КонецЕсли; 

КонецПроцедуры

Процедура СоздатьAUTHORПоДаннымUSER(Знач ДанныеПользователяХранилища)
	
	ШаблонEmail = "%1@%2";

	НовыйАвтор = Новый Структура
	НовыйАвтор.Вставить("Имя", ДанныеПользователяХранилища.NAME)
	НовыйАвтор.Вставить("Email", СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонEmail, ДанныеПользователяХранилища.NAME, ДоменПочтыДляGit);
...
Процедура ПроверитьAUTHORПоУмолчанию()
...
Функция ПолучитьПредставлениеАвтораДляGit(Знач Автор)
	...
	Возврат СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонЗаписи, Автор.Имя, Автор.Email);
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				ВыполнитьКоммитГит(КаталогРабочейКопии, СтрокаВерсии.Комментарий, ПолучитьПредставлениеАвтораДляGit(СтрокаВерсии.ГУИД_Автора), СтрокаВерсии.Дата);

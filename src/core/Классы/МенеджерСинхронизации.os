...
Процедура ИнициализоватьмAUTHORSХранилища(КаталогРабочейКопии) Экспорт
	...
	Если ФайлAUTHORS.Существует() Тогда
		...
		Сообщить("Прочитено соответсвий пользователей: " + мAUTHORS.Количество());
...
Процедура СинхронизирватьAUTHORSUSERS(Знач ДанныеUSERS, знач ФайлАвторов = Неопределено)
	...
	Для Каждого ПользовательХранилища Из ДанныеUSERS Цикл
			...
			Если Не мAUTHORS.Получить(USERID) = Неопределено Тогда
			...
			мAUTHORS.Вставить(
								USERID,
							  	СоздатьAUTHORПоДаннымUSER(ПользовательХранилища) 
							  );
	...
	Если ЕстьИзмененияВФайлеАвторов И 
		...
		Сообщить("Обнаружены изменения в файле AUTHORS");
...
Функция СоздатьAUTHORПоДаннымUSER(Знач ДанныеПользователяХранилища)
	
	ШаблонEmail = "%1@%2";
	
	NAME = ДанныеПользователяХранилища.Получить("NAME");

	НовыйАвтор = Новый Соответствие;
	НовыйАвтор.Вставить("Name", NAME);
...
Функция ПроверитьAUTHORПоУмолчанию()
	...
	Возврат ВернутьАвтораПоУмолчанию() = Неопределено; 


КонецФункции

Функция ПолучитьПредставлениеАвтораДляGit(Знач Автор)
	
	ШаблонПредставления = "%1 <%2>";
	
	Возврат СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонПредставления, Автор["FullName"], Автор["Email"]);

КонецФункции

Функция ВернутьАвтораПоГУИД(Знач USERID)

	АвторПоUSERID = мAUTHORS.Получить(USERID);

	Возврат ?(АвторПоUSERID = Неопределено,ВернутьАвтораПоУмолчанию(), АвторПоUSERID)
		
КонецФункции

Функция ВернутьАвтораПоУмолчанию()

	Возврат мAUTHORS.Получить("default");
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				АвторКоммита = ВернутьАвтораПоГУИД(СтрокаВерсии.ГУИД_Автора);  
				ПредставлениеАвтораКоммита = ПолучитьПредставлениеАвтораДляGit(АвторКоммита);

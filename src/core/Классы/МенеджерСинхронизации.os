...
Перем мСоответствиеUSERIDиЗаписиAUTHORS;
...
Процедура ВыполнитьКоммитГит(Знач КаталогРабочейКопии, Знач Комментарий, Знач Автор, Знач Дата=Неопределено, Знач УстановитьНовуюМетку, Знач Метка) Экспорт
	...
	Если УстановитьНовуюМетку Тогда
		...
		ДобавитьВКомандныйФайл(КомандныйФайл, "git tag " + Строка(Метка));
...
//
Функция ВыполнитьGitPush(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки, Знач ОтправитьTags = ложь) Экспорт
	...
	СтрокаRefsTags = "refs/tags/*:refs/tags/*";
	
	ПараметрыКоманды.Добавить(СтрШаблон(СтрокаRefsHeads,ИмяВетки));
	Если ОтправитьTags Тогда
		ПараметрыКоманды.Добавить(СтрокаRefsTags);
	КонецЕсли;	 
...
Функция КонвертироватьТаблицуВерсийИзФорматаБД(Знач ТаблицаБД)
	...
	Для Каждого СтрокаБД Из ТаблицаБД Цикл
		...
		НоваяСтрока.ГУИД_Автора = Новый УникальныйИдентификатор(СтрокаБД.USERID);
...
////////////////////
// Конвертация таблицы USERS в JSON и обратно для кодировки

Функция КонвертироватьВJSONТаблицуUSERSИзФорматаБД(Знач ТаблицаБД) Экспорт
	
	МассивДанных = Новый Массив;
	ВременныйФайлJSON = ВременныеФайлы.СоздатьФайл();
	Для Каждого СтрокаБД Из ТаблицаБД Цикл

		НоваяСтрока = Новый Структура;	
		НоваяСтрока.Вставить("NAME", СтрокаБД.NAME);
		НоваяСтрока.Вставить("USERID", СтрокаБД.USERID);
	...
	ТекстUSERS = ПарсерJSON.ЗаписатьJSON(МассивДанных,,,,,Истина);
	
	ЗаписатьДанныеВФайл(ТекстUSERS, ВременныйФайлJSON);
	
	ТекстовыеДанныеJSON = ПрочитатьДанныеИзФайла(ВременныйФайлJSON);

	массивJSON = ПарсерJSON.ПрочитатьJSON(ТекстовыеДанныеJSON);
	
	Возврат массивJSON;

КонецФункции

Функция ПолучитьПутьКФайлуAUTHORS(Знач КаталогРепо) Экспорт

	Возврат ОбъединитьПути(КаталогРепо, ИмяФайлаАвторов());
...
Процедура СконвертироватьAuthorsВФормалJSON(Знач ПутьКФайлуАвторов, Знач ДанныеUSERS) Экспорт
	...
	СтруктураUSERSПоКлючуNAME = Новый Соответствие;

	Для Каждого ЭлементUSERS Из ДанныеUSERS Цикл
		СтруктураUSERSПоКлючуNAME.Вставить(ЭлементUSERS["NAME"], ЭлементUSERS["USERID"])
	КонецЦикла;
	
	ДанныеAUTHORS = Новый Соответствие;
	...
	Попытка
		...
		Пока СтрокаФайла <> Неопределено Цикл
			...
			Если Индекс > 0 Тогда
				...
				USERID = СтруктураUSERSПоКлючуNAME.Получить(Ключ);
				Если USERID = Неопределено Тогда
					Продолжить;
				КонецЕсли;

				ИндексНачалаEmail = Найти(СтрокаФайла, "<");
				ИндексКонецEmail = Найти(СтрокаФайла, ">");

				Имя = СокрЛП(Сред(СтрокаФайла,Индекс+1, ИндексНачалаEmail - Индекс-1));
				Email = СокрЛП(Сред(СтрокаФайла, ИндексНачалаEmail+1, ИндексКонецEmail - ИндексНачалаEmail-1));
				
				НовыйАвтор = Новый Соответствие;
				
				НовыйАвтор.Вставить("Name",Ключ);
				НовыйАвтор.Вставить("FullName", Имя);
				НовыйАвтор.Вставить("Email", Email);

				ДанныеAUTHORS.Вставить(Строка(USERID), НовыйАвтор)
	...
	ЗаписатьФайлAuthors(ПутьКФайлуАвторов, ДанныеAUTHORS);

КонецПроцедуры

Процедура ПрочитатьФайлAuthors(Знач ПутьКФайлуАвторов)

	Если НЕ ЗначениеЗаполнено(ПутьКФайлуАвторов) Тогда
		ВызватьИсключение  "Не задан путь к файлу AUTHORS";
	КонецЕсли;


	Файл = Новый Файл(ПутьКФайлуАвторов);
	Если Не Файл.Существует() Тогда
		ВызватьИсключение "Файл AUTHORS не существует";
	КонецЕсли;

	ТекстФайлАвторов = ПрочитатьДанныеИзФайла(ПутьКФайлуАвторов);
	
	ПарсерJSON = Новый ПарсерJSON;
	мAUTHORS = ПарсерJSON.ПрочитатьJSON(ТекстФайлАвторов);

КонецПроцедуры

Процедура ЗаписатьФайлAuthors(Знач ФайлАвторов, Знач ДанныеAUTHORS = Неопределено)

	Если ДанныеAUTHORS = Неопределено Тогда ДанныеAUTHORS = мAUTHORS КонецЕсли;

	ПарсерJSON = Новый ПарсерJSON;
	ТекстФайлАвторов = ПарсерJSON.ЗаписатьJSON(ДанныеAUTHORS,,,,,Истина);
	Сообщить(ФайлАвторов);
	ЗаписатьДанныеВФайл(ТекстФайлАвторов, ФайлАвторов);
...
Процедура ИнициализоватьмAUTHORS() Экспорт

	Если мAUTHORS = Неопределено Тогда
	
		мAUTHORS = Новый Соответствие;
	
	КонецЕсли;

КонецПроцедуры

Процедура ИнициализоватьмAUTHORSХранилища(КаталогРабочейКопии) Экспорт

	ИнициализоватьмAUTHORS();
	ПутьКФайлуAUTHORS = ПолучитьПутьКФайлуAUTHORS(КаталогРабочейКопии);
	ФайлAUTHORS = Новый Файл(ПутьКФайлуAUTHORS);
	Если ФайлAUTHORS.Существует() Тогда
		
		ПрочитатьФайлAuthors(ПутьКФайлуAUTHORS);
		
		Лог.Отладка(СтрШаблон("Прочтено соответсвий пользователей: %1",мAUTHORS.Количество()));
				
	КонецЕсли;

КонецПроцедуры

Процедура УстановитьАвтораПоУмолчанию(Знач ИмяАвтора = Неопределено, Знач Email = Неопределено)
	
	USERID = "default";
	
	Если ИмяАвтора = Неопределено Тогда
		ИмяАвтора = "Администратор";
	КонецЕсли;

	Если Email = Неопределено Тогда
		ШаблонEmail = "%1@%2";
	
		Email = СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонEmail, ИмяАвтора, ДоменПочтыДляGitПоУмолчанию);
	КонецЕсли;

	НовыйАвтор = Новый Соответствие;
	НовыйАвтор.Вставить("Name", ИмяАвтора);
	НовыйАвтор.Вставить("FullName", ИмяАвтора);
	НовыйАвтор.Вставить("Email", Email);

	мAUTHORS.Вставить(USERID, НовыйАвтор);
...
Процедура ИнициализацияСоответствияUSERIDиЗаписиAUTHORS()

	мСоответствиеUSERIDиЗаписиAUTHORS = Новый Соответствие;
	
	Для Каждого АвторКлючЗначение Из мAUTHORS Цикл

			Если ТипЗнч(АвторКлючЗначение.Значение["UID"]) = Тип("Массив") Тогда
				
				Для Каждого ЭлементUID Из АвторКлючЗначение.Значение["UID"] Цикл
					мСоответствиеUSERIDиЗаписиAUTHORS.Вставить(ЭлементUID, АвторКлючЗначение.Ключ);
				КонецЦикла;
	
			Иначе
				мСоответствиеUSERIDиЗаписиAUTHORS.Вставить(АвторКлючЗначение.Значение["UID"], АвторКлючЗначение.Ключ);
			КонецЕсли;
	КонецЦикла;
...
Процедура СинхронизирватьAUTHORSUSERS(Знач ДанныеUSERS, знач ФайлАвторов = Неопределено)

	ЕстьИзмененияВФайлеАвторов = Ложь;
	
	Для Каждого ПользовательХранилища Из ДанныеUSERS Цикл
	
			USERID = ПользовательХранилища.Получить("USERID");
			USERKEY = ПользовательХранилища.Получить("NAME");
			
			КлючАвторВAUTHORS = ВернутьКлючАвтораПоГУИД(USERID);
			Если КлючАвторВAUTHORS = Неопределено тогда
				НайденныйПользователь = Неопределено;
			Иначе
				НайденныйПользователь = мAUTHORS.Получить(КлючАвторВAUTHORS);
			КонецЕсли;
			// Поиск по UID в глобальной переменной
			Если НайденныйПользователь = Неопределено Тогда
				Лог.Отладка(СтрШаблон("Пользователь с именем: %1 по UID (%2) не найден. Поиск будет продолжен по имени",USERKEY,USERID));
			Иначе	
				Лог.Отладка(СтрШаблон("Пользователь с именем: %1 найден по UID (%2). 
				|Найденный пользователь:	
				|	Имя: %3
				|	Полное имя: %4
				|	Email: %5", USERKEY, USERID, USERKEY, НайденныйПользователь["FullName"], НайденныйПользователь["Email"]));
				Продолжить;
			КонецЕсли;

			// Поиск по UID в глобальной переменной
			
			НайденныйПользователь = мAUTHORS.Получить(USERKEY);
			
			Если НайденныйПользователь = Неопределено Тогда
				Лог.Отладка(СтрШаблон("Пользователь с именем: %1 по имени не найден. Будет создан новая запись в AUTHORS", USERKEY));
				
				// Вообще новый пользователь
				НовыйАвтор = СоздатьНовогоАвтора(USERKEY, USERID);
				мAUTHORS.Вставить(USERKEY, НовыйАвтор);
				
			Иначе
				
				Лог.Отладка(СтрШаблон("Пользователь с именем: %1 найден по имени. Будет добавлен UID (%2) к найденному пользователю",USERKEY,USERID));
				
				МассивУИДОВ = Новый Массив;

				Если ТипЗнч(НайденныйПользователь["UID"]) = Тип("Массив") Тогда

					МассивУИДОВ = НайденныйПользователь["UID"];
				Иначе
					
					МассивУИДОВ.Добавить(НайденныйПользователь["UID"]);

				КонецЕсли;

				// Добавление нового UID в массив;
				МассивУИДОВ.Добавить(USERID);

				НайденныйПользователь["UID"] = МассивУИДОВ;

				мAUTHORS.Вставить(USERKEY, НайденныйПользователь);

				ЕстьИзмененияВФайлеАвторов = Истина;
	...
	Если ЕстьИзмененияВФайлеАвторов И 
		Не ФайлАвторов = Неопределено Тогда 
		Лог.Отладка("Обнаружены изменения в файле AUTHORS");

		ЗаписатьФайлAuthors(ФайлАвторов);
		// Повторная инициализация
		ИнициализацияСоответствияUSERIDиЗаписиAUTHORS();
	 КонецЕсли; 

КонецПроцедуры

Функция СоздатьНовогоАвтора(Знач NAME, Знач USERID)
	
	ШаблонEmail = "%1@%2";
		
	НовыйАвтор = Новый Соответствие;
	НовыйАвтор.Вставить("FullName", NAME);
	НовыйАвтор.Вставить("Email", СтроковыеФункции.ПодставитьПараметрыВСтроку(ШаблонEmail, NAME, ДоменПочтыДляGitПоУмолчанию));
	НовыйАвтор.Вставить("UID", USERID);
	
	Возврат НовыйАвтор; 

КонецФункции

Функция ПроверитьAUTHORПоУмолчанию()
	
	Возврат ВернутьКлючАвтораПоУмолчанию() = Неопределено; 

КонецФункции

Функция ПолучитьПредставлениеАвтораДляGit(Знач USERID)
	
	КлючАвторВAUTHORS = ВернутьКлючАвтораПоГУИД(USERID);
	Автор = мAUTHORS.Получить(КлючАвторВAUTHORS);
	ШаблонПредставления = "%1 <%2>";
	
	Возврат СтрШаблон(ШаблонПредставления, Автор["FullName"], Автор["Email"]);

КонецФункции

Функция ВернутьКлючАвтораПоГУИД(Знач USERID)

	АвторПоUSERID = мСоответствиеUSERIDиЗаписиAUTHORS.Получить(USERID);

	Возврат АвторПоUSERID;
		
КонецФункции

Функция ВернутьКлючАвтораПоУмолчанию()
...
Процедура СформироватьПервичныйФайлПользователейДляGit(Знач ИмяФайлаХранилища, Знач ВыходнойФайл) Экспорт
	...
	ДанныеUSERS = КонвертироватьВJSONТаблицуUSERSИзФорматаБД(ТаблицаПользователейХранилища);

	ИнициализоватьмAUTHORS();
	
	СинхронизирватьAUTHORSUSERS(ДанныеUSERS, ВыходнойФайл);
...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
	...
	ТаблицаИсторииХранилища = ПрочитатьТаблицуИсторииХранилища(ФайлХранилища);
	
	ТаблицаПользователей = ПрочитатьТаблицуПользователейХранилища(ФайлХранилища);
	МассивПользователей = КонвертироватьВJSONТаблицуUSERSИзФорматаБД(ТаблицаПользователей);
	
	// инициализация глобальных переменных
	ИнициализоватьмAUTHORSХранилища(КаталогРабочейКопии);
	ИнициализацияСоответствияUSERIDиЗаписиAUTHORS();
	
	СинхронизирватьAUTHORSUSERS(МассивПользователей, ПолучитьПутьКФайлуAUTHORS(КаталогРабочейКопии));
	...
	ПоследнняяВерсияКонфигурации = "0.0.0";

	// Поиск метки к текущей версии хранилища
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				ПредставлениеАвтораКоммита = ПолучитьПредставлениеАвтораДляGit(СтрокаВерсии.ГУИД_Автора);
				ВыполнитьКоммитГит(КаталогРабочейКопии, СтрокаВерсии.Комментарий, ПредставлениеАвтораКоммита , СтрокаВерсии.Дата, УстановитьНовуюМетку, СтрокаВерсии.Тэг);
				СчетчикКоммитов = СчетчикКоммитов + 1;

				Если ИспользоватьПромежуточныйPUSH И
					СчетчикКоммитов = КоличествоКоммитовДоPush Тогда
					СчетчикКоммитов = 0;
					ВыполнитьGitPush(КаталогРабочейКопии, URLРепозитория, ИмяВетки, АвтоматическаяУстановкаТэговПоВерсиям);
...
Процедура РазложитьМодулиПоНомеруВерсииХранилища1С(Знач КаталогРабочейКопии, Знач ФайлХранилища, Знач НомерВерсии, Знач Формат) Экспорт

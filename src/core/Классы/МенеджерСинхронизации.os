...
Процедура ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(Знач КаталогРабочейКопии, Знач ПутьКХранилищу, Знач НомерВерсии, Знач Формат, Знач ПараметрыДоступаКХранилищу)
	...
	ОчиститьЦелевойКаталогВыгрузки(КаталогРабочейКопии)
	...
	РазложитьМодули1СПоПапкамСогласноИерархииМетаданных(КаталогРабочейКопии, КаталогВыгрузки, Формат);
...
Процедура РазложитьМодулиПоНомеруВерсииХранилища1С(Знач КаталогРабочейКопии, Знач ПутьКХранилищу, Знач НомерВерсии, Знач Формат, Знач ПараметрыДоступаКХранилищу) Экспорт
	...
	Плагины.ПередВыполнениемВыгрузки(КаталогРабочейКопии, ПутьКХранилищу, НомерВерсии, Формат, ПараметрыДоступаКХранилищу);

	СтандартнаяОбработка = Истина;
	Плагины.ПриВыполненииВыгрузки(КаталогРабочейКопии, ПутьКХранилищу, НомерВерсии, Формат, ПараметрыДоступаКХранилищу, СтандартнаяОбработка);
	...
	Плагины.ПослеВыполненияВыгрузки(КаталогРабочейКопии, ПутьКХранилищу, НомерВерсии, Формат, ПараметрыДоступаКХранилищу);

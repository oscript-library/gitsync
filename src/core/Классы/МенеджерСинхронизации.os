...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
	...
	Плагины.ПослеПолученияТаблицыВерсий(ТаблицаИсторииХранилища, ПутьКХранилищу, КаталогРабочейКопии);
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Попытка
				...
				Если ИспользоватьПромежуточныйPUSH И
					...
					ВыполнитьGitPush(КаталогРабочейКопии, URLРепозитория, ИмяВетки, Ложь);
					СчетчикКоммитов = 0;
					//ОтправитьНовыеМетки  = Ложь;
				КонецЕсли;
...
Процедура УстановитьПлагины(Знач СписокПлагинов) Экспорт
	
	Плагины.ЗагрузитьПлагины(СписокПлагинов, ЭтотОбъект);
...
Плагины = Новый ИнициализацияПлагинов;

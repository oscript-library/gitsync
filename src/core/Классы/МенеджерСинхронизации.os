...
Перем ТекущаяВерсияПлатформыВыгрузки Экспорт; // Строка, используемая версия 1С.Предприятие для выгрузки в файлы
Перем ТекущийПутьКПлатформеВыгрузки Экспорт; // Строка, путь к платформе 1С.Предприятие для выгрузки в файлы
...
// Устанавливает версию платформы 1С для выгрузки конфигурации в файлы
//
// Параметры:
//   НоваяВерсияПлатформы - Строка - версия платформы 1С, используемой для выгрузки в файлы
//
// Возвращаемое значение:
//   Объект.МенеджерСинхронизации - ссылка на текущий объект класса <МенеджерСинхронизации>
//
Функция ВерсияПлатформыВыгрузки(Знач НоваяВерсияПлатформы) Экспорт

	ТекущаяВерсияПлатформыВыгрузки = НоваяВерсияПлатформы;
	Возврат ЭтотОбъект;

КонецФункции

// Устанавливает путь к платформе 1С для выгрузки конфигурации в файлы
//
// Параметры:
//   НовыйПутьКПлатформе - Строка - путь к исполняемому файлу платформы 1С.Предприятие
//
...
//   Объект.МенеджерСинхронизации - ссылка на текущий объект класса <МенеджерСинхронизации>
//
Функция ПутьКПлатформеВыгрузки(Знач НовыйПутьКПлатформе) Экспорт

	ТекущийПутьКПлатформеВыгрузки = НовыйПутьКПлатформе;
	Возврат ЭтотОбъект;
...
Процедура ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(Знач КаталогРабочейКопии,
	...
	ПрименитьПлатформуХранилищаККонфигуратору(Конфигуратор);
	...
	ПрименитьПлатформуВыгрузкиККонфигуратору(Конфигуратор);
...
Процедура ОчиститьКаталогРабочейКопии(Знач КаталогРабочейКопии) Экспорт
	...
	СоответствиеИменФайловДляПропуска.Вставить(".gitignore", Истина);
	СоответствиеИменФайловДляПропуска.Вставить(".gitattributes", Истина);
	СоответствиеИменФайловДляПропуска.Вставить(".hooks", Истина);	
...
Процедура ПрименитьПлатформуХранилищаККонфигуратору(Знач Конфигуратор)
	...
	Если ЗначениеЗаполнено(ТекущийПутьКПлатформе) Тогда
		...
		Конфигуратор.ПутьКПлатформе1С(ТекущийПутьКПлатформе);
		Лог.Информация("Для работы с хранилищем используется путь к платформе 1С <%1>", ТекущийПутьКПлатформе);
		Возврат;

	КонецЕсли;

	Конфигуратор.ИспользоватьВерсиюПлатформы(ТекущаяВерсияПлатформы);
	Лог.Информация("Для работы с хранилищем используется платформа 1С версии <%1>", ТекущаяВерсияПлатформы);

КонецПроцедуры

Процедура ПрименитьПлатформуВыгрузкиККонфигуратору(Знач Конфигуратор)

	Если ЗначениеЗаполнено(ТекущийПутьКПлатформеВыгрузки) Тогда

		Если Не ФС.ФайлСуществует(ТекущийПутьКПлатформеВыгрузки) Тогда

			ВызватьИсключение СтрШаблон("Исполняемый файл платформы 1С для выгрузки <%1> не найден",
				ТекущийПутьКПлатформеВыгрузки);

		КонецЕсли;

		Конфигуратор.ПутьКПлатформе1С(ТекущийПутьКПлатформеВыгрузки);
		Лог.Информация("Для выгрузки используется путь к платформе 1С <%1>", ТекущийПутьКПлатформеВыгрузки);
		Возврат;

	КонецЕсли;

	Если ЗначениеЗаполнено(ТекущаяВерсияПлатформыВыгрузки) Тогда

		Конфигуратор.ИспользоватьВерсиюПлатформы(ТекущаяВерсияПлатформыВыгрузки);
		Лог.Информация("Для выгрузки используется платформа 1С версии <%1>", ТекущаяВерсияПлатформыВыгрузки);
...
// 	   * Тег                 - Строка - метка версии в хранилище
...
Функция ПрочитатьТаблицуИсторииХранилища(Знач ПутьКХранилищу, Знач НачальнаяВерсия = 1) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Для Каждого СтрокаВерсииХранилища Из ТаблицаВерсийХранилища Цикл
			...
			СтрокаВерсии.Тег 			= ""; // теги в отчете не предоставляются
...
Процедура ДополнитьТаблицуВерсийИменамиАвторов(Знач ТаблицаВерсий, Знач ТаблицаПользователей)
	...
	Для Каждого Строка Из ТаблицаВерсий Цикл
		...
		Иначе
			Строка.Автор = СокрЛП(СтрокаПользователя.Автор);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриСозданииОбъекта()
	...
	ТекущаяВерсияПлатформыВыгрузки = "";
	ТекущийПутьКПлатформеВыгрузки = "";

...
Процедура СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
	...
	Если СтрокаТекущейВерсии <> Неопределено Тогда
		ПоследнняяВерсияКонфигурации = СтрокаТекущейВерсии.Тег;
	КонецЕсли;
	...
	Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
		...
		Если СтрокаВерсии <> Неопределено Тогда
			...
			Если ПоследнняяВерсияКонфигурации <> СтрокаВерсии.Тег Тогда
			...
			Попытка
				...
				ВыполнитьКоммитГит(КаталогРабочейКопии, СтрокаВерсии.Комментарий, ПредставлениеАвтораКоммита , СтрокаВерсии.Дата, УстановитьНовуюМетку, СтрокаВерсии.Тег);

...
Функция ПолучитьПутьКФайлуВключенныхПлагинов()
	...
	Если Не ФайлАктивныхПлагинов.Существует() Тогда
		РабочийКаталог = ОбъединитьПути(ТекущийКаталог(), ".gitsync");
		ФС.ОбеспечитьКаталог(РабочийКаталог);
		Возврат  ОбъединитьПути(РабочийКаталог, ИмяФайлаВключенныхПлагинов());
	КонецЕсли;
...
Процедура ЗаписатьВключенныеПлагины() Экспорт
	...
	Для каждого КлючЗначение Из ИндексПлагинов Цикл
		...
		Плагин = КлючЗначение.Значение;
		Если Плагин.Включен() Тогда
			ВключенныеПлагины.Вставить(Плагин.Имя(), Истина);
		КонецЕсли;

	КонецЦикла;
	
	ТекстФайла = ПарсерJSON.ЗаписатьJson(ВключенныеПлагины);
...
Процедура ЗаписатьФайл(Знач ИмяФайла, ТекстФайла)
	...
	Запись.УстановитьСтроку(ТекстФайла);
	Запись.Закрыть();
...
Процедура ПриСозданииОбъекта(Знач ЗначениеКаталогПлагинов = Неопределено)
	...
	КаталогСистемныхБиблиотек = ПолучитьЗначениеСистемнойНастройки("lib.system");
	...
	Иначе
		КаталогЗависимостей = КаталогСистемныхБиблиотек;
	КонецЕсли;
